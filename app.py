...
def _match_explain_text(mode):
    ...
    school_line = (
        ...
        "问卷用户优先于隐私用户，同档再看问卷相似度，"
        "只给你优先级最高的 1 人；历史上配过的人双向永不再配；"
        "页面不展示匹配度分数，只给契合点与破冰话题"
        + (
    ...
    return {
        ...
        "steps": [
            ...
            "2. 排序：双方填问卷 > 一方填问卷 > 双方隐私；同档再用余弦相似度（仅内部排序，不展示分数）。",
            "3. 一票否决：婚姻、孩子出现明确相反意愿，或出轨观、吸烟接受度差异过大时直接跳过。",
            "4. 择偶取向：双方都愿意匹配对方的性别才进入候选。",
            "5. 黑名单双向生效；历史上配过的人双向永不再配（含已失效的旧卡，不会复活）。",
            "6. 跨校需双方都勾选且总闸开启。",
            "7. 每人每周最多参与 1 次新匹配（发起或被配都算），不会反复抢走同一人。",
            "8. 揭晓时互见学校邮箱与附加联系方式；破冰话题帮你开口（避开出轨/婚姻等开场雷区）。",
        ],
