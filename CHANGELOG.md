...
### Fixed
- **问卷页体验**：量表题从「默认中间值滑块」改为 1–5 点选；未作答不计入进度，提交前校验并高亮缺题
- **答案键类型**：`answers` / `important_qids` 读写统一规范为 int，修复一票否决与匹配理由因 JSON 字符串键失效
- **重新匹配**：已有 Match 会更新分数并返回结果（不再静默空列表）；邮件仅通知新匹配
- **种子数据**：修正兴趣选项写错（咖啡馆/民谣/看书等），支持 `python seed.py --refresh`
- **问卷 API**：拒绝不足 20 题的提交；过滤非法 multi 选项

### Improved
- 进度条 sticky、已答/未答状态、★ 重要标记、移动端 Likert 布局
- 匹配共同点增加兴趣交集描述
